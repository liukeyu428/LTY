...
from models.EEGNet import CEDL
...
def train(args):
    ...
    data_sampler = BalanceIDSampler(source_X_list1, target_X, source_y_list1, target_y, args.batch_size)#1：8加载batch
    args.batch_size = data_sampler.revised_batch_size
    data_num = data_sampler.__len__()
    train_data = EEGDataset(source_X_list1, target_X, source_y_list1, target_y, data_num)#改
    ...
    backbone = CEDL(source_X_list[0].shape[-2], source_X_list[0].shape[-1], args.class_num,len(list1))
    ...
    for epoch in range(start_epoch, args.epochs):
        if args.early_stop and stop_train.early_stop and epoch>500:
            ...
            break
        steps = train_one_epoch1(trainLoader, backbone, device, opt, cls_criterion1,
                                cls_criterion2,
