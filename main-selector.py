...
from models.EEGNet import  CSNet
...
def train(args):
    ...
    # ------------------------------------------------model setting----------------------------------------------------
    backbone = CSNet(source_X_list[0].shape[-2], source_X_list[0].shape[-1], args.class_num)
    ...
    for epoch in range(start_epoch, args.epochs):
        if args.early_stop and stop_train.early_stop:
            ...
            break
        steps = train1(trainLoader, backbone, device, opt, cls_criterion1,
                                cls_criterion2,
                                start_time, epoch,args)

        avg_acc, avg_loss= evaluate(testLoader, backbone, device, cls_criterion1, start_time,
                                               epoch, args)
        ...
        if 1.0 == best_acc:
            ...
            break
    avg_acc1, avg_loss1 = evaluate_other(trainLoader, backbone, device, cls_criterion1, start_time,
                                         epoch, args)
